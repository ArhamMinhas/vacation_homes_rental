...
export default function PropertyFilters({ variant = "hero" }: HomeSearchProps) {
  ...
  return (
    ...
    >
      <motion.div
        className="bg-white rounded-full flex items-stretch overflow-hidden border border-gray-100/90"
        style={{ boxShadow: "0 8px 30px rgba(0,0,0,0.12), 0 2px 8px rgba(0,0,0,0.06)" }}
        whileHover={{ boxShadow: "0 16px 40px rgba(0,0,0,0.18), 0 4px 12px rgba(0,0,0,0.08)" }}
      ...
      >
        ...
        <div className="flex-1 hidden sm:flex flex-col px-6 py-3 hover:bg-gray-50/60 transition-colors border-r border-gray-100 min-w-0">
          <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-0.5">Check In</span>
          <div className="flex items-center gap-2">
            <input
              type="date"
              value={checkIn}
      ...
      <div className="flex sm:hidden gap-2 mt-2">
        <div className="flex-1 bg-white border border-gray-100 rounded-2xl px-4 py-2.5 shadow-md">
          <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-0.5">Check In</span>
          <input
            type="date"
            value={checkIn}
            min={today}
        ...
        <div className="flex-1 bg-white border border-gray-100 rounded-2xl px-4 py-2.5 shadow-md">
          <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-0.5">Guests</span>
          <input
            type="number"
            min={1}
            max={20}
