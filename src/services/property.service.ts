...
/** Active properties with optional filters and pagination — public listing. */
export async function getFilteredProperties(
  filters: PropertyFilters = {}
): Promise<PropertyPage> {
  const supabase = await createClient()

  const pageSize = Math.min(filters.pageSize ?? 12, 48)  // cap at 48
  ...
  let query = supabase
    .from("properties")
    .select(WITH_IMAGES, { count: "exact" })
    .eq("is_active", true)
    .order("created_at", { ascending: false })
    .range(from, to)
...
/** Paginated version for the admin properties table. */
export async function getPaginatedPropertiesAdmin(
  page = 1,
  pageSize = 15
): Promise<PropertyPage> {
  const supabase = createAdminClient()
